...
TEST(TestRmwImplementationProtoCpp, ImplementationIdentifierOnly)
{
  EXPECT_STREQ(rmw_get_implementation_identifier(), "rmw_ecal_proto_cpp");
}

TEST(TestRmwImplementationProtoCpp, InitOptionsLifecycle)
{
  ...
  EXPECT_STREQ(init_options.implementation_identifier, "rmw_ecal_proto_cpp");

  rmw_init_options_t dst = rmw_get_zero_initialized_init_options();
  ret = rmw_init_options_copy(&init_options, &dst);
  EXPECT_EQ(ret, RMW_RET_OK);
  EXPECT_STREQ(dst.implementation_identifier, "rmw_ecal_proto_cpp");
