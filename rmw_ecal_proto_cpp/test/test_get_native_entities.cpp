...
class TestNativeEntitiesProtoCpp : public ::testing::Test
{
protected:
  void SetUp() override
  {
...
TEST_F(TestNativeEntitiesProtoCpp, get_domain_participant)
{
  EXPECT_STREQ("rmw_ecal_proto_cpp", node->implementation_identifier);
  
  const char * original_identifier = node->implementation_identifier;
  node->implementation_identifier = "not-matching-rmw-identifier";
  EXPECT_STRNE("rmw_ecal_proto_cpp", node->implementation_identifier);
  node->implementation_identifier = original_identifier;
}

TEST_F(TestNativeEntitiesProtoCpp, get_datawriter)
{
  ...
  EXPECT_STREQ("rmw_ecal_proto_cpp", pub->implementation_identifier);

  EXPECT_EQ(RMW_RET_OK, rmw_destroy_publisher(node, pub));
}

TEST_F(TestNativeEntitiesProtoCpp, get_datareader)
{
  const rosidl_message_type_support_t * ts =
    rosidl_typesupport_introspection_cpp::get_message_type_support_handle<std_msgs::msg::String>();
  ...
  EXPECT_STREQ("rmw_ecal_proto_cpp", sub->implementation_identifier);

  EXPECT_EQ(RMW_RET_OK, rmw_destroy_subscription(node, sub));
}

TEST_F(TestNativeEntitiesProtoCpp, get_service)
{
  ...
  EXPECT_STREQ("rmw_ecal_proto_cpp", srv->implementation_identifier);

  EXPECT_EQ(RMW_RET_OK, rmw_destroy_service(node, srv));
}

TEST_F(TestNativeEntitiesProtoCpp, get_client)
{
  const rosidl_service_type_support_t * ts =
    rosidl_typesupport_introspection_cpp::get_service_type_support_handle<std_srvs::srv::Empty>();
  ...
  EXPECT_STREQ("rmw_ecal_proto_cpp", client->implementation_identifier);
